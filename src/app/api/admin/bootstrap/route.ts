...
export async function GET() {
  ...
  const [content, requests, settings, directors, admins, pretinha] = await Promise.all([
  ...
  return NextResponse.json({ admin, content: visibleContent, requests: visibleRequests, directors: directors.rows, users: admins.rows, pretinha: pretinha.rows, settings: settings.rows[0] ? JSON.parse(String(settings.rows[0].value)) : null, settingsId: settings.rows[0]?.$id });
}
