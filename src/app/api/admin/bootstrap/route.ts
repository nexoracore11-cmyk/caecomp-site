...
export async function GET() {
  ...
  const [content, requests, settings, directors, admins, pretinha, stores, campaigns, calendar] = await Promise.all([
    ...
    (isMaster(admin)||admin.permissions.includes("stores")||admin.permissions.includes("stores_manage")) ? tables().listRows({databaseId:config.databaseId,tableId:"store_profiles",queries:[Query.limit(100)],total:false}) : Promise.resolve({rows:[]}),
  ...
  const visibleContent = content.rows.filter((row) => {
    ...
    if (row.module === "department_posts") return canManageDepartment(admin, String(row.category || ""));
